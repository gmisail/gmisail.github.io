...
$hxClasses["BuildingRegistry"] = BuildingRegistry;
BuildingRegistry.__name__ = true;
...
var kvell2D_Game = function() {
};
$hxClasses["kvell2D.Game"] = kvell2D_Game;
kvell2D_Game.__name__ = true;
kvell2D_Game.prototype = {
	begin: function() {
	}
	,render: function() {
		kvell2D_Manager.engine.get_scene().render();
	}
	,update: function() {
		kvell2D_Manager.engine.get_scene().update();
	}
	,__class__: kvell2D_Game
};
var Main = function() {
	kvell2D_Game.call(this);
};
$hxClasses["Main"] = Main;
Main.__name__ = true;
Main.main = function() {
	kvell2D_Manager.init(640,640);
	kvell2D_Manager.engine.start(new Main());
	Main.view = new View();
	Main.menu = new Menu();
};
Main.__super__ = kvell2D_Game;
Main.prototype = $extend(kvell2D_Game.prototype,{
	begin: function() {
		Main.view = new View();
		kvell2D_Manager.engine.set_scene(Main.menu);
	}
	,__class__: Main
});
Math.__name__ = true;
var kvell2D_Scene = function() {
	this.objects = [];
};
$hxClasses["kvell2D.Scene"] = kvell2D_Scene;
kvell2D_Scene.__name__ = true;
kvell2D_Scene.prototype = {
	objects: null
	,add: function(o) {
		this.objects.push(o);
	}
	,remove: function(o) {
		HxOverrides.remove(this.objects,o);
	}
	,addLayer: function(l) {
		var _g = 0;
		var _g1 = l.contents;
	...
	,render: function() {
		...
		while(_g < _g1.length) {
			...
			++_g;
			if(object.visible) {
				object.render(kvell2D_Manager.engine.buffer);
			}
		}
	}
	,dispose: function() {
	}
	,__class__: kvell2D_Scene
};
var Menu = function() {
	kvell2D_Scene.call(this);
};
$hxClasses["Menu"] = Menu;
Menu.__name__ = true;
Menu.__super__ = kvell2D_Scene;
Menu.prototype = $extend(kvell2D_Scene.prototype,{
	start: null
	,begin: function() {
		this.start = new kvell2D_graphics_Text(0,100,"Press Space to Start",kvell2D_Manager.assets.font("slkscr"));
		this.start.set_size(24);
		this.start.center(true,true);
		this.add(this.start);
		kvell2D_Scene.prototype.begin.call(this);
	}
	,update: function() {
		if(kvell2D_Manager.keyboard.SPACE) {
			kvell2D_Manager.engine.set_scene(Main.view);
		}
		kvell2D_Scene.prototype.update.call(this);
	}
	,__class__: Menu
});
var Mode = $hxClasses["Mode"] = { __ename__ : true, __constructs__ : ["ROAD","BUILDING","WATER","ERASE","SELECT","MOVE"] };
Mode.ROAD = ["ROAD",0];
Mode.ROAD.toString = $estr;
Mode.ROAD.__enum__ = Mode;
Mode.BUILDING = ["BUILDING",1];
Mode.BUILDING.toString = $estr;
Mode.BUILDING.__enum__ = Mode;
Mode.WATER = ["WATER",2];
Mode.WATER.toString = $estr;
Mode.WATER.__enum__ = Mode;
Mode.ERASE = ["ERASE",3];
Mode.ERASE.toString = $estr;
Mode.ERASE.__enum__ = Mode;
Mode.SELECT = ["SELECT",4];
Mode.SELECT.toString = $estr;
Mode.SELECT.__enum__ = Mode;
Mode.MOVE = ["MOVE",5];
Mode.MOVE.toString = $estr;
Mode.MOVE.__enum__ = Mode;
...
Stats.__name__ = true;
Stats.addBuilding = function(b) {
	Stats.overall += b.influence;
};
Stats.addDorm = function(d) {
	BuildingRegistry.dorms.push(d);
	Stats.maxPopulation += d.size;
};
Stats.addBuildingUpgrade = function(b) {
	Stats.money -= b.value / 4 | 0;
	b.quality += 1;
	Stats.overall += b.quality;
};
Stats.calculatePop = function() {
	Stats.population = Stats.overall;
};
Stats.update = function() {
	Stats.calculatePop();
};
Stats.debug = function() {
	haxe_Log.trace("\nCurrent Population: " + Stats.population + "\nMaximum Population: " + Stats.maxPopulation + "\n",{ fileName : "Stats.hx", lineNumber : 58, className : "Stats", methodName : "debug"});
};
Stats.prototype = {
	__class__: Stats
};
...
var Tile = function(id,name,value) {
...
$hxClasses["TileManager"] = TileManager;
TileManager.__name__ = true;
TileManager.init = function() {
	TileManager.overlay = new kvell2D_graphics_BasicGraphic(0,0,0,0);
	TileManager.building = new Building();
};
TileManager.update = function(s,mode,map) {
	if(s.uiVisible) {
		return;
	}
	var xt = kvell2D_Manager.mouse.position.x / 16 | 0;
	var yt = kvell2D_Manager.mouse.position.y / 16 | 0;
	if(kvell2D_Manager.keyboard.Q) {
		mode = Mode.SELECT;
	}
	if(mode == Mode.BUILDING) {
		if(!TileManager.isOverlay) {
			s.add(TileManager.overlay);
			TileManager.isOverlay = true;
		}
		TileManager.overlay.transform.x = xt * 16;
		TileManager.overlay.transform.y = yt * 16;
		TileManager.overlay.transform.width = TileManager.building.w * 16;
		TileManager.overlay.transform.height = TileManager.building.h * 16;
	} else {
		s.remove(TileManager.overlay);
		TileManager.placing = false;
		TileManager.isOverlay = false;
	}
	if(kvell2D_Manager.mouse.down) {
		if(mode == Mode.ROAD) {
			if(map.getTile(xt,yt) == TileRegistry.grass.id) {
				map.setTile(xt,yt,TileRegistry.road.id);
				Stats.money -= TileRegistry.road.value;
			}
		} else if(mode == Mode.BUILDING) {
			TileManager.placing = true;
			var area = TileManager.building.w * TileManager.building.h;
			var id = 0;
			var _g1 = 0;
			var _g = TileManager.building.src[0].length;
			while(_g1 < _g) {
				var x = _g1++;
				var _g3 = 0;
				var _g2 = TileManager.building.src.length;
				while(_g3 < _g2) if(map.getTile(xt + x,yt + _g3++) == TileRegistry.grass.id) {
					++id;
				}
			}
			if(area == id) {
				var _g11 = 0;
				var _g4 = TileManager.building.src[0].length;
				while(_g11 < _g4) {
					var x1 = _g11++;
					var _g31 = 0;
					var _g21 = TileManager.building.src.length;
					while(_g31 < _g21) {
						var y = _g31++;
						if(map.getTile(xt + x1,yt + y) == TileRegistry.grass.id) {
							map.setTile(xt + x1,yt + y,TileManager.building.src[y][x1]);
						}
					}
				}
				if(js_Boot.__instanceof(TileManager.building,buildings_Dorm)) {
					Stats.addDorm(new buildings_Dorm());
				}
				Stats.money -= TileManager.building.value;
			}
			mode = Mode.SELECT;
		} else if(mode == Mode.WATER) {
			var et = map.getTile(xt,yt);
			var _g5 = 0;
			var _g12 = TileRegistry.tiles;
			while(_g5 < _g12.length) {
				var t = _g12[_g5];
				++_g5;
				if(t.id == et && et == TileRegistry.grass.id) {
					map.setTile(xt,yt,TileRegistry.water.id);
					Stats.money -= TileRegistry.water.value / 2 | 0;
				}
			}
		} else if(mode == Mode.ERASE) {
			var et1 = map.getTile(xt,yt);
			if(TileRegistry.isBuilding(et1)) {
				return;
			}
			map.setTile(xt,yt,TileRegistry.grass.id);
			var _g6 = 0;
			var _g13 = TileRegistry.tiles;
			while(_g6 < _g13.length) {
				var t1 = _g13[_g6];
				++_g6;
				if(t1.id == et1) {
					Stats.money += t1.value / 2 | 0;
				}
			}
		}
	}
...
$hxClasses["TileRegistry"] = TileRegistry;
TileRegistry.__name__ = true;
TileRegistry.isBuilding = function(id) {
	var _g1 = 0;
	var _g = TileRegistry.buildingTiles.length;
	while(_g1 < _g) if(TileRegistry.buildingTiles[_g1++].id == id) {
		return true;
	}
	return false;
};
TileRegistry.init = function() {
	TileRegistry.tiles = [];
	TileRegistry.tiles.push(TileRegistry.road);
	TileRegistry.tiles.push(TileRegistry.grass);
	TileRegistry.tiles.push(TileRegistry.water);
	TileRegistry.buildingTiles = [];
	TileRegistry.buildingTiles.push(TileRegistry.topLeft);
	TileRegistry.buildingTiles.push(TileRegistry.top);
	TileRegistry.buildingTiles.push(TileRegistry.topRight);
	TileRegistry.buildingTiles.push(TileRegistry.left);
	TileRegistry.buildingTiles.push(TileRegistry.middle);
	TileRegistry.buildingTiles.push(TileRegistry.right);
	TileRegistry.buildingTiles.push(TileRegistry.bottomLeft);
	TileRegistry.buildingTiles.push(TileRegistry.bottom);
	TileRegistry.buildingTiles.push(TileRegistry.bottonRight);
};
TileRegistry.prototype = {
	__class__: TileRegistry
};
var Type = function() { };
$hxClasses["Type"] = Type;
Type.__name__ = true;
...
$hxClasses["View"] = View;
View.__name__ = true;
View.__super__ = kvell2D_Scene;
View.prototype = $extend(kvell2D_Scene.prototype,{
	map: null
	,mode: null
	,ui: null
	,cameraSpeed: null
	...
	,begin: function() {
		...
		var font = kvell2D_Manager.assets.font("slkscr");
		this.map = new kvell2D_graphics_Tilemap(kvell2D_util_TilemapUtil.fill(2,40,50),kvell2D_Manager.assets.image("lowfy"),16,16);
		this.add(this.map);
		this.ui = new zui_Zui(font,17,16,0,1.0);
		this.add(new kvell2D_util_FPS(font));
		kvell2D_Scene.prototype.begin.call(this);
	}
	,update: function() {
		Stats.update();
		if(kvell2D_Manager.keyboard.I) {
			this.uiVisible = true;
		}
		if(kvell2D_Manager.keyboard.ESC) {
			this.uiVisible = false;
		}
		TileManager.update(this,this.mode,this.map);
		Stats.debug();
		kvell2D_Scene.prototype.update.call(this);
	}
	,render: function() {
		kvell2D_Scene.prototype.render.call(this);
		var g = kvell2D_Manager.engine.buffer.get_g2();
		if(this.uiVisible) {
			this.ui.begin(g);
			if(this.ui.window("0",32,32,kvell2D_Manager.screen.get_width() - 64,kvell2D_Manager.screen.get_height() - 64,0)) {
				if(this.ui.node("0",Stats.$name,0,true)) {
					this.ui.indent();
					Stats.$name = this.ui.textInput("0",Stats.$name,"Name");
					this.ui.text("$" + Stats.money);
					this.ui.text(Stats.acceptance + "%");
					this.ui.text(Stats.population + " students");
					this.ui.text(Stats.maxPopulation + " max");
					this.ui.separator();
					if(this.ui.button("Erase")) {
					...
					this.ui.separator();
					if(this.ui.node("1","Tiles")) {
						this.ui.indent();
						if(this.ui.node("2","Basic")) {
							this.ui.indent();
							if(this.ui.button("Road - $2")) {
								this.mode = Mode.ROAD;
							}
							if(this.ui.button("Water - $1")) {
								this.mode = Mode.WATER;
							}
							this.ui.unindent();
						}
						if(this.ui.node("3","Other")) {
							this.ui.indent();
							this.ui.unindent();
						}
						this.ui.unindent();
					}
					if(this.ui.node("4","Buildings")) {
						this.ui.indent();
						if(this.ui.node("5","Dorms")) {
							this.ui.indent();
							this.ui.text("[ 20 students ]");
							this.ui.text("Purchase: $1000");
							if(this.ui.button("Buy")) {
								this.mode = Mode.BUILDING;
								TileManager.building = new buildings_Dorm();
							}
							this.ui.unindent();
						}
						if(this.ui.node("6","Library")) {
							this.ui.indent();
							this.ui.text("Purchase: $1000");
							this.ui.text("Upgrade: $250");
							if(this.ui.button("Buy")) {
								if(BuildingRegistry.library == null) {
									this.mode = Mode.BUILDING;
									BuildingRegistry.library = new buildings_Library();
									TileManager.building = BuildingRegistry.library;
								} else {
									kvell2D_Manager.log.send("You already have a library.");
								}
							}
							if(this.ui.button("Upgrade")) {
								if(BuildingRegistry.library != null) {
									Stats.addBuildingUpgrade(BuildingRegistry.library);
								}
							}
							this.ui.unindent();
						}
						if(this.ui.node("7","Science")) {
							this.ui.indent();
							this.ui.text("Physical Science");
							this.ui.text("Purchase: $2000");
							this.ui.text("Upgrade: $250");
							if(this.ui.button("Buy")) {
								this.mode = Mode.BUILDING;
								TileManager.building = new buildings_Physics();
							}
							this.ui.button("Upgrade");
							this.ui.separator();
							this.ui.text("Natural Science");
							this.ui.text("Purchase: $2000");
							this.ui.text("Upgrade: $250");
							if(this.ui.button("Buy")) {
								this.mode = Mode.BUILDING;
								TileManager.building = new buildings_Library();
							}
							this.ui.button("Upgrade");
							this.ui.separator();
							this.ui.text("Engineering & Computer Science");
							this.ui.text("Purchase: $2000");
							this.ui.text("Upgrade: $250");
							if(this.ui.button("Buy")) {
								this.mode = Mode.BUILDING;
						...
						if(this.ui.node("8","Arts")) {
							this.ui.text("Theater");
							this.ui.text("Purchase: $4000");
							this.ui.text("Upgrade: $250");
							if(this.ui.button("Buy")) {
								this.mode = Mode.BUILDING;
								TileManager.building = new buildings_Library();
...
var kvell2D_Object = function() {
	this.visible = true;
	this.active = true;
	this.transform = new kvell2D_Transform();
};
$hxClasses["kvell2D.Object"] = kvell2D_Object;
kvell2D_Object.__name__ = true;
kvell2D_Object.prototype = {
	transform: null
	,active: null
	,visible: null
	,center: function(x,y) {
		if(x) {
			this.transform.x = kvell2D_Manager.screen.get_width() / 2 - this.transform.width / 2 | 0;
		}
		if(y) {
			this.transform.y = kvell2D_Manager.screen.get_height() / 2 - this.transform.height / 2 | 0;
		}
	}
	,onScreen: function() {
		return true;
	...
	,__class__: kvell2D_Object
};
var kvell2D_graphics_Graphic = function(x,y,image) {
	if(y == null) {
		y = 0;
	}
	if(x == null) {
	...
	this.scaleY = 1;
	this.scaleX = 1;
	kvell2D_Object.call(this);
	this.transform.x = x;
	this.transform.y = y;
	this.transform.velocity = new kha_math_Vector2(0,0);
	this.transform.acceleration = new kha_math_Vector2(0,0);
...
$hxClasses["kvell2D.graphics.Graphic"] = kvell2D_graphics_Graphic;
kvell2D_graphics_Graphic.__name__ = true;
kvell2D_graphics_Graphic.__super__ = kvell2D_Object;
kvell2D_graphics_Graphic.prototype = $extend(kvell2D_Object.prototype,{
	image: null
	,scaleX: null
	,scaleY: null
	,frame: null
	,load: function(i) {
		this.image = i;
		this.transform.width = this.image.get_width();
		this.transform.height = this.image.get_height();
		this.scaleX = this.transform.width;
		this.scaleY = this.transform.height;
		if(this.frame == null) {
			this.frame = new kvell2D_graphics_Frame(this.image,this.transform.width,this.transform.height);
		}
	}
	,render: function(i) {
		kvell2D_Object.prototype.render.call(this,i);
		...
		this.transform.x += this.transform.velocity.x * kvell2D_Manager.time.delta;
		this.transform.y += this.transform.velocity.y * kvell2D_Manager.time.delta;
		this.onScreen();
		i.get_g2().drawScaledSubImage(this.image,this.frame.x,this.frame.y,this.frame.width,this.frame.height,this.transform.x,this.transform.y,this.scaleX,this.scaleY);
	}
	,stop: function() {
	...
	,setAcceleration: function(x,y) {
		...
		this.setAccelerationY(y);
	}
	,createFrames: function(frameWidth,frameHeight) {
		this.frame = new kvell2D_graphics_Frame(this.image,frameWidth,frameHeight);
	}
	,setFrameDimensions: function(x,y) {
		this.frame.width = x;
		this.frame.height = y;
	}
	,setFrame: function(x,y) {
		this.frame.x = x * this.frame.width | 0;
		this.frame.y = y * this.frame.height | 0;
	}
	,animate: function(order,seconds) {
	}
	,setSize: function(x,y) {
		this.scaleX = x;
		this.scaleY = y;
	}
	,__class__: kvell2D_graphics_Graphic
});
var buildings_Athletics = function() {
	this.imageBased = true;
	kvell2D_graphics_Graphic.call(this);
	this.img = kvell2D_Manager.assets.image("athletics");
	this.load(this.img);
};
$hxClasses["buildings.Athletics"] = buildings_Athletics;
buildings_Athletics.__name__ = true;
buildings_Athletics.__super__ = kvell2D_graphics_Graphic;
buildings_Athletics.prototype = $extend(kvell2D_graphics_Graphic.prototype,{
	img: null
	,imageBased: null
	,__class__: buildings_Athletics
});
var buildings_Dorm = function() {
	this.size = 20;
	Building.call(this);
	this.influence = 5;
	this.value = 1000;
	this.quality = 1;
	this.src = [[4,5,6],[12,4,14],[20,21,22]];
	this.w = this.src[0].length;
	this.h = this.src.length;
};
$hxClasses["buildings.Dorm"] = buildings_Dorm;
buildings_Dorm.__name__ = true;
buildings_Dorm.__super__ = Building;
buildings_Dorm.prototype = $extend(Building.prototype,{
	size: null
	,__class__: buildings_Dorm
});
var buildings_Library = function() {
	Building.call(this);
	this.influence = 10;
	this.value = 1000;
	this.quality = 1;
	this.src = [[4,5,6],[12,9,14],[20,21,22]];
	this.w = this.src[0].length;
	this.h = this.src.length;
};
$hxClasses["buildings.Library"] = buildings_Library;
buildings_Library.__name__ = true;
buildings_Library.__super__ = Building;
buildings_Library.prototype = $extend(Building.prototype,{
	__class__: buildings_Library
});
var buildings_Natural = function() {
	Building.call(this);
	this.influence = 15;
	this.value = 1000;
	this.quality = 1;
	this.src = [[4,5,6],[12,10,14],[20,21,22]];
	this.w = this.src[0].length;
	this.h = this.src.length;
};
$hxClasses["buildings.Natural"] = buildings_Natural;
buildings_Natural.__name__ = true;
buildings_Natural.__super__ = Building;
buildings_Natural.prototype = $extend(Building.prototype,{
	__class__: buildings_Natural
});
var buildings_Physics = function() {
	Building.call(this);
	this.influence = 15;
	this.value = 1000;
	this.quality = 1;
...
$hxClasses["buildings.Physics"] = buildings_Physics;
buildings_Physics.__name__ = true;
buildings_Physics.__super__ = Building;
buildings_Physics.prototype = $extend(Building.prototype,{
	__class__: buildings_Physics
});
...
var kha_ImageList = function() {
	this.names = ["items","lowfy","poke","spaceship","tileset","title","wabbit","tri","village"];
	this.villageDescription = { files : ["village.png"], original_height : 128, type : "image", original_width : 128, name : "village"};
	this.villageName = "village";
	this.village = null;
	this.triDescription = { files : ["tri.png"], original_height : 16, type : "image", original_width : 16, name : "tri"};
	this.triName = "tri";
	this.tri = null;
	this.wabbitDescription = { files : ["wabbit.png"], original_height : 37, type : "image", original_width : 26, name : "wabbit"};
	this.wabbitName = "wabbit";
	this.wabbit = null;
	this.titleDescription = { files : ["title.png"], original_height : 200, type : "image", original_width : 200, name : "title"};
	this.titleName = "title";
	this.title = null;
	this.tilesetDescription = { files : ["tileset.png"], original_height : 32, type : "image", original_width : 32, name : "tileset"};
	this.tilesetName = "tileset";
	this.tileset = null;
	this.spaceshipDescription = { files : ["spaceship.png"], original_height : 32, type : "image", original_width : 32, name : "spaceship"};
	this.spaceshipName = "spaceship";
	this.spaceship = null;
	this.pokeDescription = { files : ["poke.png"], original_height : 560, type : "image", original_width : 368, name : "poke"};
	this.pokeName = "poke";
	this.poke = null;
	this.lowfyDescription = { files : ["lowfy.png"], original_height : 128, type : "image", original_width : 128, name : "lowfy"};
	this.lowfyName = "lowfy";
	this.lowfy = null;
	this.itemsDescription = { files : ["items.png"], original_height : 128, type : "image", original_width : 128, name : "items"};
...
kha_ImageList.prototype = {
	...
	,itemsLoad: function(done) {
		kha_Assets.loadImage("items",function(image) {
			done();
		});
	}
	,itemsUnload: function() {
		this.items.unload();
		this.items = null;
	}
	,lowfy: null
	,lowfyName: null
	,lowfyDescription: null
	,lowfyLoad: function(done) {
		kha_Assets.loadImage("lowfy",function(image) {
			done();
		});
	}
	,lowfyUnload: function() {
		this.lowfy.unload();
		this.lowfy = null;
	}
	,poke: null
	...
	,pokeLoad: function(done) {
		kha_Assets.loadImage("poke",function(image) {
			done();
		});
	}
	,pokeUnload: function() {
	...
	,spaceship: null
	,spaceshipName: null
	,spaceshipDescription: null
	,spaceshipLoad: function(done) {
		kha_Assets.loadImage("spaceship",function(image) {
			done();
		});
	}
	,spaceshipUnload: function() {
		this.spaceship.unload();
		this.spaceship = null;
	}
	,tileset: null
	,tilesetName: null
	...
	,titleLoad: function(done) {
		kha_Assets.loadImage("title",function(image) {
			done();
		});
	}
	,titleUnload: function() {
		this.title.unload();
		this.title = null;
	}
	,wabbit: null
	,wabbitName: null
	,wabbitDescription: null
	,wabbitLoad: function(done) {
		kha_Assets.loadImage("wabbit",function(image) {
			done();
		});
	}
	,wabbitUnload: function() {
		this.wabbit.unload();
		this.wabbit = null;
	}
	,tri: null
	,triName: null
	,triDescription: null
	,triLoad: function(done) {
		kha_Assets.loadImage("tri",function(image) {
			done();
		});
	}
	,triUnload: function() {
	...
	,villageLoad: function(done) {
		kha_Assets.loadImage("village",function(image) {
			done();
		});
	}
	,villageUnload: function() {
		this.village.unload();
		this.village = null;
	}
	,names: null
...
var kha_BlobList = function() {
	this.names = ["Overworld_mp3","map_csv","map_txt"];
	this.map_txtDescription = { files : ["map.txt"], type : "blob", name : "map_txt"};
	this.map_txtName = "map_txt";
	this.map_txt = null;
	this.map_csvDescription = { files : ["map.csv"], type : "blob", name : "map_csv"};
	this.map_csvName = "map_csv";
	this.map_csv = null;
	this.Overworld_mp3Description = { files : ["Overworld.mp3"], type : "blob", name : "Overworld_mp3"};
	this.Overworld_mp3Name = "Overworld_mp3";
	this.Overworld_mp3 = null;
};
$hxClasses["kha.BlobList"] = kha_BlobList;
kha_BlobList.__name__ = true;
kha_BlobList.prototype = {
	Overworld_mp3: null
	,Overworld_mp3Name: null
	,Overworld_mp3Description: null
	,Overworld_mp3Load: function(done) {
		kha_Assets.loadBlob("Overworld_mp3",function(blob) {
			done();
		});
	}
	,Overworld_mp3Unload: function() {
		this.Overworld_mp3.unload();
		this.Overworld_mp3 = null;
	}
	,map_csv: null
	,map_csvName: null
	,map_csvDescription: null
	,map_csvLoad: function(done) {
		kha_Assets.loadBlob("map_csv",function(blob) {
			done();
		});
	}
	,map_csvUnload: function() {
		this.map_csv.unload();
		this.map_csv = null;
	}
	,map_txt: null
	,map_txtName: null
	,map_txtDescription: null
	,map_txtLoad: function(done) {
		kha_Assets.loadBlob("map_txt",function(blob) {
			done();
		});
	}
	,map_txtUnload: function() {
		this.map_txt.unload();
		this.map_txt = null;
	}
	,names: null
...
var kha_FontList = function() {
	this.names = ["KarmaFuture","slkscr"];
	this.slkscrDescription = { files : ["slkscr.ttf"], type : "font", name : "slkscr"};
	this.slkscrName = "slkscr";
	this.slkscr = null;
	this.KarmaFutureDescription = { files : ["KarmaFuture.ttf"], type : "font", name : "KarmaFuture"};
	this.KarmaFutureName = "KarmaFuture";
	this.KarmaFuture = null;
};
$hxClasses["kha.FontList"] = kha_FontList;
kha_FontList.__name__ = true;
kha_FontList.prototype = {
	KarmaFuture: null
	,KarmaFutureName: null
	,KarmaFutureDescription: null
	,KarmaFutureLoad: function(done) {
		kha_Assets.loadFont("KarmaFuture",function(font) {
			done();
		});
	}
	,KarmaFutureUnload: function() {
		this.KarmaFuture.unload();
		this.KarmaFuture = null;
	}
	,slkscr: null
	,slkscrName: null
	,slkscrDescription: null
	,slkscrLoad: function(done) {
		kha_Assets.loadFont("slkscr",function(font) {
			done();
		});
	}
	,slkscrUnload: function() {
		this.slkscr.unload();
		this.slkscr = null;
	}
	,names: null
		...
		if(kha_Scheduler.activeTimeTask.next <= frameEnd) {
			...
			if(kha_Scheduler.activeTimeTask.active && kha_Scheduler.activeTimeTask.task()) {
				if(kha_Scheduler.activeTimeTask.period > 0 && (kha_Scheduler.activeTimeTask.duration == 0 || kha_Scheduler.activeTimeTask.duration >= kha_Scheduler.activeTimeTask.start + kha_Scheduler.activeTimeTask.next)) {
					...
					kha_Scheduler.archiveTimeTask(kha_Scheduler.activeTimeTask);
				}
			} else {
...
kha_Scheduler.archiveTimeTask = function(timeTask) {
};
...
kha_network_Session.prototype = {
	...
	,addController: function(controller) {
		haxe_Log.trace("Adding controller id " + controller._id(),{ fileName : "Session.hx", lineNumber : 77, className : "kha.network.Session", methodName : "addController"});
		...
		case 3:
			...
			while(index < bytes.length) {
				...
				case 66:
					...
					++index;
					haxe_Log.trace("Bool: " + (value == null?"null":"" + value),{ fileName : "Session.hx", lineNumber : 209, className : "kha.network.Session", methodName : "receive"});
					args.push(value);
					break;
				case 70:
					var value1 = bytes.getDouble(index);
					index += 8;
					haxe_Log.trace("Float: " + value1,{ fileName : "Session.hx", lineNumber : 214, className : "kha.network.Session", methodName : "receive"});
					args.push(value1);
					break;
				case 73:
					var value2 = bytes.getInt32(index);
					index += 4;
					haxe_Log.trace("Int: " + value2,{ fileName : "Session.hx", lineNumber : 219, className : "kha.network.Session", methodName : "receive"});
					args.push(value2);
					break;
				case 83:
					...
					haxe_Log.trace("String: " + str,{ fileName : "Session.hx", lineNumber : 229, className : "kha.network.Session", methodName : "receive"});
					args.push(str);
					break;
				default:
					haxe_Log.trace("Unknown argument type.",{ fileName : "Session.hx", lineNumber : 232, className : "kha.network.Session", methodName : "receive"});
				}
			}
...
var kvell2D_Camera = function() {
	this.following = false;
	this.boundries = false;
	this.scaleY = 1;
	this.scaleX = 1;
	this.viewport = new kvell2D_math_Rectangle(0,0,kvell2D_Manager.screen.get_width(),kvell2D_Manager.screen.get_height());
	this.matrix = new kha_math_FastMatrix3(1,0,-this.viewport.x,0,1,-this.viewport.y,0,0,1);
};
$hxClasses["kvell2D.Camera"] = kvell2D_Camera;
kvell2D_Camera.__name__ = true;
kvell2D_Camera.prototype = {
	matrix: null
	,scaleX: null
	,scaleY: null
	,viewport: null
	...
	,setBounds: function(x,y,width,height) {
		this.bounds = new kvell2D_math_Rectangle(x,y,width,height);
		this.boundries = true;
	}
	,update: function() {
	}
	,__class__: kvell2D_Camera
};
var kvell2D_Engine = function() {
	this.alive = true;
	this.visible = true;
	this.fps = 60;
};
$hxClasses["kvell2D.Engine"] = kvell2D_Engine;
kvell2D_Engine.__name__ = true;
kvell2D_Engine.prototype = {
	fps: null
	,scene: null
	,_begin: null
	,_render: null
	...
	,start: function(game) {
		var _gthis = this;
		if(kvell2D_Manager.screen.get_title() == null) {
			kvell2D_Manager.screen.set_title("Basic");
		}
		kha_System.init({ title : kvell2D_Manager.screen.get_title(), width : kvell2D_Manager.screen.get_width(), height : kvell2D_Manager.screen.get_height()},function() {
			kvell2D_Manager.initInput();
			_gthis.camera = new kvell2D_Camera();
			kha_Assets.loadEverything(function() {
				_gthis._begin = $bind(game,game.begin);
				_gthis._render = $bind(game,game.render);
				_gthis._update = $bind(game,game.update);
				_gthis.buffer = kha_Image.createRenderTarget(kvell2D_Manager.screen.get_width(),kvell2D_Manager.screen.get_height());
				_gthis.begin();
				kha_System.notifyOnRender($bind(_gthis,_gthis.render));
				kha_Scheduler.addTimeTask($bind(_gthis,_gthis.update),0,1 / _gthis.fps);
			});
		});
	}
	,begin: function() {
	...
	,render: function(framebuffer) {
		if(this.visible) {
			var _graphics = this.buffer.get_g2();
			_graphics.begin(null,kvell2D_Manager.screen.get_color());
			_graphics.pushTransformation(new kha_math_FastMatrix3(1,0,-this.camera.viewport.x,0,1,-this.camera.viewport.y,0,0,1));
			this._render();
			_graphics.popTransformation();
			_graphics.end();
			framebuffer.get_g2().begin();
			kha_Scaler.scale(this.buffer,framebuffer,kha_System.get_screenRotation());
	...
	,set_scene: function(scene) {
		this.camera.reset();
		this.scene = scene;
		this.get_scene().begin();
		return scene;
	}
	,get_scene: function() {
		if(this.scene == null) {
			return new kvell2D_Scene();
		} else {
			return this.scene;
		}
	}
	,__class__: kvell2D_Engine
};
var kvell2D_assets_File = function() {
};
$hxClasses["kvell2D.assets.File"] = kvell2D_assets_File;
kvell2D_assets_File.__name__ = true;
kvell2D_assets_File.prototype = {
	image: function(name) {
		return Reflect.field(kha_Assets.images,name);
	}
	,file: function(name) {
		return Reflect.field(kha_Assets.blobs,name);
	}
	,sound: function(name) {
		return Reflect.field(kha_Assets.sounds,name);
	}
	,font: function(name) {
		return Reflect.field(kha_Assets.fonts,name);
	}
	,video: function(name) {
		return Reflect.field(kha_Assets.videos,name);
	}
	,__class__: kvell2D_assets_File
};
var kvell2D_audio_Audio = function() {
	this.on = true;
};
$hxClasses["kvell2D.audio.Audio"] = kvell2D_audio_Audio;
kvell2D_audio_Audio.__name__ = true;
kvell2D_audio_Audio.prototype = {
	on: null
	,play: function(s,loop) {
		if(loop == null) {
			loop = false;
		}
		kha_audio2_Audio1.play(s.base,false);
	}
	,playMusic: function(s) {
		kha_audio2_Audio1.play(s.base,true);
	}
	,__class__: kvell2D_audio_Audio
};
var kvell2D_input_Keyboard = function() {
	this.SPACE = false;
	this.BACK = false;
	this.RIGHT = false;
	this.LEFT = false;
...
$hxClasses["kvell2D.input.Keyboard"] = kvell2D_input_Keyboard;
kvell2D_input_Keyboard.__name__ = true;
kvell2D_input_Keyboard.prototype = {
	get: function() {
		if(kha_input_Keyboard.get() != null) {
			kha_input_Keyboard.get().notify($bind(this,this._keyDown),$bind(this,this._keyUp));
		}
	}
	,A: null
	,B: null
	...
	,__class__: kvell2D_input_Keyboard
};
var kvell2D_debug_Log = function() {
};
$hxClasses["kvell2D.debug.Log"] = kvell2D_debug_Log;
kvell2D_debug_Log.__name__ = true;
kvell2D_debug_Log.prototype = {
	send: function(text) {
		haxe_Log.trace(text + "\n",{ fileName : "Log.hx", lineNumber : 10, className : "kvell2D.debug.Log", methodName : "send"});
	}
	,statistics: function(s) {
		this.send("Added: " + s.objects.length);
	}
	,__class__: kvell2D_debug_Log
};
var kvell2D_input_Mouse = function() {
	this.leftDown = false;
	this.rightDown = false;
	this.down = false;
	this.position = new kha_math_FastVector2(0,0);
	this.distanceFromLast = new kha_math_FastVector2(0,0);
};
$hxClasses["kvell2D.input.Mouse"] = kvell2D_input_Mouse;
kvell2D_input_Mouse.__name__ = true;
kvell2D_input_Mouse.prototype = {
	down: null
	,rightDown: null
	,leftDown: null
	,position: null
	...
	,_onMouseMove: function(x,y,mx,my) {
		this.position.x = x;
		this.position.y = y;
		this.distanceFromLast.x = mx;
		this.distanceFromLast.y = my;
	}
	,get: function() {
		kha_input_Mouse.get().notify($bind(this,this._onMouseDown),$bind(this,this._onMouseUp),$bind(this,this._onMouseMove),null);
	}
	,__class__: kvell2D_input_Mouse
};
var kvell2D_physics_Physics = function() {
};
$hxClasses["kvell2D.physics.Physics"] = kvell2D_physics_Physics;
kvell2D_physics_Physics.__name__ = true;
kvell2D_physics_Physics.prototype = {
	...
	,collide: function(a,b) {
		if(this.overlap(a,b)) {
			this.collisionSteps(a,b);
			return true;
		}
		return false;
	}
	,collisionSteps: function(a,b) {
	}
	,__class__: kvell2D_physics_Physics
};
var kvell2D_math_Random = function() {
};
$hxClasses["kvell2D.math.Random"] = kvell2D_math_Random;
kvell2D_math_Random.__name__ = true;
kvell2D_math_Random.prototype = {
	getScreenCoordinate: function() {
		return new kha_math_Vector2(this.get(kvell2D_Manager.screen.get_width()),this.get(kvell2D_Manager.screen.get_height()));
	}
	,get: function(range) {
		if(range <= 1) {
			haxe_Log.trace("If the range is <= 1, it will always return 1.",{ fileName : "Random.hx", lineNumber : 17, className : "kvell2D.math.Random", methodName : "get"});
		}
		return Std.random(range);
	}
	,__class__: kvell2D_math_Random
};
var kvell2D_Screen = function() {
};
$hxClasses["kvell2D.Screen"] = kvell2D_Screen;
kvell2D_Screen.__name__ = true;
kvell2D_Screen.prototype = {
	width: null
	,height: null
	,drawWidth: null
	,drawHeight: null
	...
	,color: null
	,center: null
	,setSize: function(width,height,drawWidth,drawHeight) {
		this.set_width(width);
		this.set_height(height);
		if(drawWidth == null) {
			this.drawWidth = this.get_width();
		} else {
			this.drawWidth = drawWidth;
		}
		if(drawHeight == null) {
			this.drawHeight = this.get_height();
		} else {
	...
	,get_center: function() {
		return new kha_math_Vector2(this.get_width() / 2,this.get_height() / 2);
	}
	,__class__: kvell2D_Screen
};
var kvell2D_Time = function() {
	this.times = [];
	this.reset();
};
$hxClasses["kvell2D.Time"] = kvell2D_Time;
kvell2D_Time.__name__ = true;
kvell2D_Time.prototype = {
	delta: null
	,last: null
	,fps: null
	,times: null
	...
	,update: function() {
		var current = kha_Scheduler.time();
		this.delta = current - this.last;
		this.last = current;
		this.times.push(current);
		while(this.times[0] < current - 1) this.times.shift();
		this.fps = this.times.length;
	}
	,__class__: kvell2D_Time
};
var kvell2D_Manager = function() {
};
$hxClasses["kvell2D.Manager"] = kvell2D_Manager;
kvell2D_Manager.__name__ = true;
kvell2D_Manager.init = function(w,h) {
	kvell2D_Manager.screen.setSize(w,h);
};
kvell2D_Manager.initInput = function() {
	kvell2D_Manager.keyboard.get();
	kvell2D_Manager.mouse.get();
};
kvell2D_Manager.prototype = {
	__class__: kvell2D_Manager
};
var kvell2D_Transform = function() {
	this.acceleration = new kha_math_Vector2(0,0);
	this.velocity = new kha_math_Vector2(0,0);
	this.height = 0;
	this.width = 0;
	this.y = 0;
	this.x = 0;
};
$hxClasses["kvell2D.Transform"] = kvell2D_Transform;
kvell2D_Transform.__name__ = true;
kvell2D_Transform.prototype = {
	x: null
	,y: null
	,width: null
	,height: null
	,velocity: null
	,acceleration: null
	,__class__: kvell2D_Transform
};
var kvell2D_audio_Sound = function(base,volume) {
	if(volume == null) {
		volume = 1;
	}
	this.base = base;
...
$hxClasses["kvell2D.audio.Sound"] = kvell2D_audio_Sound;
kvell2D_audio_Sound.__name__ = true;
kvell2D_audio_Sound.prototype = {
	base: null
	,volume: null
	...
	,__class__: kvell2D_audio_Sound
};
var kvell2D_graphics_BasicGraphic = function(x,y,w,h) {
	kvell2D_Object.call(this);
	this.transform.x = x;
	this.transform.y = y;
	this.transform.width = w;
	this.transform.height = h;
	this.color = kha__$Color_Color_$Impl_$.White;
};
$hxClasses["kvell2D.graphics.BasicGraphic"] = kvell2D_graphics_BasicGraphic;
kvell2D_graphics_BasicGraphic.__name__ = true;
kvell2D_graphics_BasicGraphic.__super__ = kvell2D_Object;
kvell2D_graphics_BasicGraphic.prototype = $extend(kvell2D_Object.prototype,{
	color: null
	,render: function(i) {
		kvell2D_Object.prototype.render.call(this,i);
		var g = i.get_g2();
	...
	,__class__: kvell2D_graphics_BasicGraphic
});
var kvell2D_graphics_Frame = function(baseImage,width,height) {
...
$hxClasses["kvell2D.graphics.Frame"] = kvell2D_graphics_Frame;
kvell2D_graphics_Frame.__name__ = true;
kvell2D_graphics_Frame.prototype = {
	baseImage: null
	,width: null
	,height: null
	,x: null
	,y: null
	,__class__: kvell2D_graphics_Frame
};
var kvell2D_graphics_Layer = function() {
	this.contents = [];
};
$hxClasses["kvell2D.graphics.Layer"] = kvell2D_graphics_Layer;
kvell2D_graphics_Layer.__name__ = true;
kvell2D_graphics_Layer.prototype = {
	contents: null
	,attach: function(object) {
		this.contents.push(object);
	}
	,detach: function(object) {
		HxOverrides.remove(this.contents,object);
	}
	,__class__: kvell2D_graphics_Layer
};
var kvell2D_graphics_Text = function(x,y,text,font) {
	kvell2D_Object.call(this);
	this.transform.x = x;
	this.transform.y = y;
	this.color = kha__$Color_Color_$Impl_$.White;
	...
	this.font = font;
	this.set_size(16);
	this.transform.width = this.font.width(this.get_size(),text) | 0;
	this.transform.height = this.font.height(this.get_size()) | 0;
};
$hxClasses["kvell2D.graphics.Text"] = kvell2D_graphics_Text;
kvell2D_graphics_Text.__name__ = true;
kvell2D_graphics_Text.__super__ = kvell2D_Object;
kvell2D_graphics_Text.prototype = $extend(kvell2D_Object.prototype,{
	color: null
	,text: null
	,font: null
	,size: null
	...
	,render: function(i) {
		...
		g.set_fontSize(this.get_size());
		g.set_color(this.color);
		g.drawString(this.text,this.transform.x,this.transform.y);
	}
	,get_size: function() {
		return this.size;
	}
	,set_size: function(size) {
		this.size = size;
		this.transform.width = this.font.width(size,this.text) | 0;
		this.transform.height = this.font.height(size) | 0;
		return size;
	}
	,__class__: kvell2D_graphics_Text
});
var kvell2D_graphics_Tilemap = function(data,tilesetImage,tileWidth,tileHeight) {
	this.tileHeight = 0;
	this.tileWidth = 0;
	this.columns = 0;
	this.rows = 0;
	...
	this.tileset = new kvell2D_graphics_Tileset(tilesetImage,tileWidth,tileHeight);
};
$hxClasses["kvell2D.graphics.Tilemap"] = kvell2D_graphics_Tilemap;
kvell2D_graphics_Tilemap.__name__ = true;
kvell2D_graphics_Tilemap.__super__ = kvell2D_Object;
kvell2D_graphics_Tilemap.prototype = $extend(kvell2D_Object.prototype,{
	data: null
	,tileset: null
	,tilesetImage: null
	,rows: null
	...
	,render: function(i) {
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var x = _g3++;
				var tile = this.tileset.getTile(this.data[x][y]);
				...
				g.drawSubImage(this.tilesetImage,y * this.tileHeight,x * this.tileWidth,tile.x * this.tileWidth,tile.y * this.tileHeight,this.tileWidth,this.tileHeight);
			}
		}
	}
	,load: function(data) {
	...
	,loadFromString: function(input) {
		...
		while(_g1 < rows) {
			...
			while(_g3 < cols) {
				var x = _g3++;
				this.data[y][x] = Std.parseInt(col[x]);
			}
		}
	...
	,__class__: kvell2D_graphics_Tilemap
});
var kvell2D_graphics_Tileset = function(base,tw,th) {
	this.base = base;
	this.tileWidth = tw;
	this.tileHeight = th;
	this.width = base.get_width();
	this.height = base.get_height();
	this.widthInTiles = this.width / this.tileWidth | 0;
	this.heightInTiles = this.height / this.tileHeight | 0;
	var data = [];
	var id = 0;
	var _g1 = 0;
	var _g = this.heightInTiles;
	while(_g1 < _g) {
		var y = _g1++;
		data[y] = [];
		var _g3 = 0;
		var _g2 = this.widthInTiles;
		while(_g3 < _g2) {
			data[y][_g3++] = id;
			++id;
		}
...
$hxClasses["kvell2D.graphics.Tileset"] = kvell2D_graphics_Tileset;
kvell2D_graphics_Tileset.__name__ = true;
kvell2D_graphics_Tileset.prototype = {
	base: null
	,data: null
	,tileWidth: null
	,tileHeight: null
	...
	,getTile: function(id) {
		...
		return new kha_math_Vector2(-1,-1);
	}
	,__class__: kvell2D_graphics_Tileset
};
var kvell2D_math_Rectangle = function(x,y,width,height) {
	this.x = x;
	this.y = y;
	this.width = width;
	this.height = height;
};
$hxClasses["kvell2D.math.Rectangle"] = kvell2D_math_Rectangle;
kvell2D_math_Rectangle.__name__ = true;
kvell2D_math_Rectangle.prototype = {
	x: null
	,y: null
	,width: null
	,height: null
	,__class__: kvell2D_math_Rectangle
};
var kvell2D_util_FPS = function(font) {
	kvell2D_graphics_Text.call(this,0,0,"FPS: 0",font);
	this.set_size(16);
};
$hxClasses["kvell2D.util.FPS"] = kvell2D_util_FPS;
kvell2D_util_FPS.__name__ = true;
kvell2D_util_FPS.__super__ = kvell2D_graphics_Text;
kvell2D_util_FPS.prototype = $extend(kvell2D_graphics_Text.prototype,{
	update: function() {
		kvell2D_graphics_Text.prototype.update.call(this);
		this.text = "FPS: " + kvell2D_Manager.time.fps;
	}
	,__class__: kvell2D_util_FPS
});
var kvell2D_util_TilemapUtil = function() {
};
$hxClasses["kvell2D.util.TilemapUtil"] = kvell2D_util_TilemapUtil;
kvell2D_util_TilemapUtil.__name__ = true;
kvell2D_util_TilemapUtil.fill = function(id,w,h) {
	var m = [];
	var _g1 = 0;
	while(_g1 < w) {
		var x = _g1++;
		m[x] = [];
		var _g3 = 0;
		while(_g3 < h) m[x][_g3++] = id;
	}
	return m;
};
kvell2D_util_TilemapUtil.prototype = {
	__class__: kvell2D_util_TilemapUtil
};
var zui_Id = function() { };
...
zui_Zui.prototype = {
	...
	,endWindow: function() {
		...
		if(state.redraws > 0 || zui_Zui.isScrolling || zui_Zui.isTyping) {
			...
			if(fullHeight < this._windowH || state.layout == 1) {
				...
				if(this.inputStarted && this.getInputInRect(this._windowX + this._windowW - 8 * zui_Zui.SCALE,barY,8 * zui_Zui.SCALE,barH)) {
					state.scrolling = true;
					zui_Zui.isScrolling = true;
				}
				if(state.scrolling) {
					this.scroll(this.inputDY,fullHeight);
				...
				this.g.fillRect(this._windowW - 8 * zui_Zui.SCALE - this.scrollAlign,barY,8 * zui_Zui.SCALE,barH);
			}
	...
	,textInput: function(id,text,label) {
		...
		if(this.textSelectedId == id) {
			...
			this.g.set_color(-3487289);
			this.g.fillRect(this._x + this.cursorPixelX,this._y + this.cursorY * (30 * zui_Zui.SCALE),1,30 * zui_Zui.SCALE * 0.9);
...
BuildingRegistry.dorms = [];
Stats.$name = "Tycoon U";
Stats.money = 10000;
Stats.acceptance = 0;
Stats.maxPopulation = 0;
Stats.population = 0;
Stats.cost = 20000;
Stats.overall = 0;
TileManager.placing = false;
TileManager.isOverlay = false;
TileRegistry.road = new Tile(1,"road",2);
TileRegistry.grass = new Tile(2,"grass",0);
TileRegistry.water = new Tile(3,"water",1);
TileRegistry.topLeft = new Tile(4,"building_top_left",0);
TileRegistry.top = new Tile(5,"building_top",0);
TileRegistry.topRight = new Tile(6,"building_top_right",0);
TileRegistry.left = new Tile(12,"building_left",0);
TileRegistry.middle = new Tile(13,"building_middle",0);
TileRegistry.right = new Tile(14,"building_right",0);
TileRegistry.bottomLeft = new Tile(20,"building_bottom_left",0);
TileRegistry.bottom = new Tile(21,"building_bottom",0);
TileRegistry.bottonRight = new Tile(22,"building_bottom_right",0);
...
kvell2D_Manager.engine = new kvell2D_Engine();
kvell2D_Manager.keyboard = new kvell2D_input_Keyboard();
kvell2D_Manager.mouse = new kvell2D_input_Mouse();
kvell2D_Manager.physics = new kvell2D_physics_Physics();
kvell2D_Manager.time = new kvell2D_Time();
kvell2D_Manager.log = new kvell2D_debug_Log();
kvell2D_Manager.audio = new kvell2D_audio_Audio();
kvell2D_Manager.random = new kvell2D_math_Random();
kvell2D_Manager.screen = new kvell2D_Screen();
kvell2D_Manager.assets = new kvell2D_assets_File();
zui_Id.posId = 0;
zui_Id.windowId = 0;
zui_Id.nodeId = 0;
